...
--group thats scaled
local ScaledGroup = models.models.root
...
local files = toJson(listFiles(nil, true))
local FOXCameraPath = files:match('"([^"]-FOXCamera[^"]-)"')
---@class CameraAPI
local FOXCamera = FOXCameraPath and require(FOXCameraPath)
local CommandLibPath = files:match('"([^"]-CommandLib[^"]-)"')
---@class CommandLib
local CommandLib = CommandLibPath and require(CommandLibPath)

if not FOXCamera and host:isHost() then
	printJson(toJson({
		text =
		"Hey, this script doesn’t change the camera position — please use FoxCamera to move the camera if wanted.",
		color =
		"red"
	}))
end
...
local function HostChangeScale()
	...
	if FOXCamera and FOXCamera.getCamera() then
		FOXCamera.getCamera().scale = ModelScale
	end
end
...
function events.tick()
	...
	if tick_counter >= 200 / 5 then
		...
		if FOXCamera and FOXCamera.getCamera() and host:isHost() then
			FOXCamera.getCamera().scale = ModelScale
		end
...
if CommandLib and host:isHost() then
	CommandLib:createCommand("scale")
